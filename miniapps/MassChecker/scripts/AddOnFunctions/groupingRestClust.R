groupingRestClust <- function(outdir, fileIn, cores=1, scanmode, ppm=2){
  ...
  withProgress(message = "Peak grouping",{
    ...
    groups <- mzClustGeneric(as.matrix(peakmat), 
                             mzppm=ppm)
    #groups$mat <- as.data.table(groups$mat)
    # --- get intensities --
    outlist = as.data.table(outlist)
    # --------------------------
    progress <<- function(i) setProgress(value = 0.5 + (i / length(groups$idx))/2,
                                         detail = paste("Gathering mass values..."))
    opts <- list(progress=progress)
    # --- loop 2 ---
    outrows <- foreach(i=1:length(groups$idx), .options.snow=opts, .export = c("outlist", 
                                                                               "groups",
                                                                               "groupNames"),
                   .packages = "data.table",
                   .verbose = T) %dopar% {
