...
output$ggplot_font_size_example <- plotly::renderPlotly({
  p = ggplot2::ggplot(data = mtcars, aes(x = mpg, 
                                         y = disp)) + 
    ggplot2::geom_line() + 
    ggplot2::geom_point() +  
    gbl$functions$plot.themes[[lcl$aes$theme]](base_size = input$ggplot_font_size) +
