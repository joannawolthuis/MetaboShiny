shiny::observeEvent(input$enrich_plot_pathway, {
  ...
  pathway_match = which(curr_pathway == mSet$analSet$enrich$path.all$name)
  is_kegg = grepl("kegg", mSet$analSet$enrich$path.lib)

  
  if(length(pathway_match) > 0 & nrow(enrich$current) > 0){
    
    pw.code = mSet$analSet$enrich$path.all$code[pathway_match]
    ...
    direction_result_txt <- paste0(Hmisc::capitalize(upname),":",
    ...
    output$enrich_curr_direction_txt <- shiny::renderText(direction_result_txt)
    
    if(is_kegg){
      if(!file.exists(paste0(species, pw.code, ".qs"))){
        #download.kegg(species = species,
        #                        pathway.id = pw.code)
      ...
      message("Not KEGG, visualisation not available.")
      output$enrich_pathway <- NULL
    }
