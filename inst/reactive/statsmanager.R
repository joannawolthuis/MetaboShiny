               ...
               ml = {
                 try({
                   ...
                   for(settings in ml_queue$jobs){
                     ...
                     curr = data.table::as.data.table(switch(pickedTbl, 
                     ...
                     rownames(curr) <- rownames(mSet$dataSet$norm)
                     ...
                     if(!is.null(settings$ml_train_subset)){
                       if(unique(settings$ml_train_subset) %in% c("split","all")){
                         settings$ml_train_subset <- NULL
                       }
                     }
                     if(!is.null(settings$ml_test_subset)){
                       if(unique(settings$ml_test_subset)%in% c("split","all")){
                         settings$ml_test_subset <- NULL
                       }
                     }
                     
                     batch_sampling = settings$ml_batch_sampling
                     batches = settings$ml_batch_covars
                     
                     if(settings$ml_samp_distr != " "){
                       spl.name = stringr::str_split(settings$ml_samp_distr, " - ")[[1]]
                     ...
                     needed_for_subset <- c(settings$ml_train_subset[1],
                                            settings$ml_test_subset[1])
                     ...
                     config <- config[, unique(c(settings$ml_include_covars,
                                                 if(length(batches)>0) settings$ml_batch_covars else c(),
                     ...
                     if(mSet$settings$exp.type %in% c("2f", "t1f")){
                       ...
                       cls <- "cls" #if(settings$ml_run_on_norm) "cls" else "orig.cls"
                       if(nrow(config)==0){
                     ...
                     if(!is.null(settings$ml_train_subset) | !is.null(settings$ml_test_subset)){
                       ...
                       train_idx = NULL
                       if(!is.null(settings$ml_test_subset)){
                         test_idx = which(t[[settings$ml_test_subset[1]]] == settings$ml_test_subset[2])
                       }
                       if(!is.null(settings$ml_train_subset)){
                         train_idx = which(t[[settings$ml_train_subset[1]]] == settings$ml_train_subset[2])
                       }
                     ...
                     if(length(batches) > 0 & batch_sampling != "none"){
                       ...
                       if(settings$ml_batch_size_sampling){
                       ...
                       if(length(batches) == 1){
                         if(settings$ml_folds != "LOOCV"){
                           if(as.numeric(settings$ml_folds) <= 5){
                             group_fac = config[split == "train",][[batches]]
                             ml_folds = min(c(as.numeric(settings$ml_folds), length(unique(group_fac))))
                       ...
                       if(settings$ml_sampling != "none"){
                         ...
                         labelCol = which(colnames(training_data) == "label")
                         size_per_group = if(settings$ml_sampling == "down") min(table(l[[labelCol]])) else max(table(l[[labelCol]]))
                         r = switch(settings$ml_sampling,
                                    up = upsample.adj(l, as.factor(l$label), maxClass = size_per_group),
                                    rose = {
                                      resampled = ROSE::ROSE(label ~ ., 
                                                             data = {
                       ...
                       config = rbind(test.config, 
                                      train.config)
                       removeCols = setdiff(needed_for_subset, settings$ml_include_covars)
                     ...
                     if(length(batches) > 0 ){
                       alsoRemove = settings$ml_batch_covars[sapply(settings$ml_batch_covars, function(b)
                         !(b %in% settings$ml_include_covars))]
                     ...
                     goes = as.numeric(settings$ml_attempts)
                     ...
                     if(settings$ml_method %not in% names(mSet$analSet$ml)){
