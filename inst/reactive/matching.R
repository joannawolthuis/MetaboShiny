      ...
      shiny::withProgress({
        ...
        matches = pbapply::pblapply(blocks, function(mzs){
          if("cmmmediator" %in% db_list){
            res.online <- MetaDBparse::searchMZonline(mz = my_selection$mz, 
                                                      mode = MetaboShiny::getIonMode(my_selection$mz, 
                                                                        lcl$paths$patdb),
            ...
            res.online <- data.table::rbindlist(lapply(1:nrow(res.online), function(i){
              ...
              try({
                ...
                adj = MetaDBparse::doAdduct("", res.online$baseformula, charge = 0, query_adduct = add, adduct_table=adducts)
          ...
          if(predict){
            res.predict = MetaDBparse::getPredicted(mz = as.numeric(my_selection$mz), 
                                                    ppm = as.numeric(mSet$ppm),
                                                    mode = MetaboShiny::getIonMode(my_selection$mz, 
                                                                                   lcl$paths$patdb))
            if(("pubchem" %in% db_list | "chemspider" %in% db_list) & nrow(res.predict) > 0){
              ...
              res.predict <- rbind(results_nosmi, results_withsmi)
            }else{
