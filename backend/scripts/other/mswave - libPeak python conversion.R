...
cwt <- function(data, wavelet, widths){
  output = matrix(c(0), nrow = length(widths), ncol = length(data))
  #output = matrix(0 * length(widths), 0 * length(data))
  for(ind in seq_along(widths)){
    width <- widths[ind]
