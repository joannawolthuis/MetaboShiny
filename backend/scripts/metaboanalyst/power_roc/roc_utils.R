...
# Compute lasso frequency when users request, not part of default
# The lasso is disabled as it is likely the cause of memory leak before server shutdown
# ==> msg: There are more than 500 variables and n<m; You may wish to restart and set use.Gram=FALSE
GetLassoFreqs <- function(){
    if(ncol(dataSet$norm) < 500){
        lassoFreq <- try(GetROCLassoFreq(dataSet$norm, dataSet$cls));
        if(class(lassoFreq) == "try-error"){
            err.msg <<- "Unknown errors occured during computing lasso!";
            lassoFreq <- rep(0, ncol(dataSet$norm));
        }
    }else{
        err.msg <<- "Too many variables (>500) with small sample size, computing aborted!";
        lassoFreq <- rep(0, ncol(dataSet$norm));
    }
    names(lassoFreq) <- colnames(dataSet$norm);
