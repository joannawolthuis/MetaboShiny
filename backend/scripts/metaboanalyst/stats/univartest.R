...
GetTtCmpdInxs<-function(){
    return(1:length(analSet$tt$p.log));
}
...
GetMaxTtInxs <- function(){
    which.max(analSet$tt$p.log);
}

GetTtestSigFileName <- function(){
    analSet$tt$sig.nm;
}
...
Volcano.Anal<-function(paired=FALSE,fcthresh,cmpType, percent.thresh, nonpar=F, threshp, equal.var=TRUE, pval.type="raw"){
...
ANOVA.Anal<-function(nonpar=F, thresh=0.05, post.hoc="fisher"){
    ...
    if(nonpar){
     ...
     current.msg <<- paste(c("A total of", sum(inx.imp), "significant features were found."), collapse=" ");
     if(sig.num> 0){
...
GetAovSigNum <- function(){
    return(analSet$aov$sig.num);
}
...
GetAnovaCmpdInxs<-function(){
    return(1:length(analSet$aov$p.log));
}
GetMaxAnovaInx <- function(){
    which.max(analSet$aov$p.log);
}

GetAnovaSigFileName <- function(){
    analSet$aov$sig.nm;
}
