...
Read.BatchCSVdata<-function(filePath, format, label){
    ...
    if(!is.null(dataSet$batch.cls)){
       if(!setequal(levels(cls.nms), levels(dataSet$batch.cls[[1]]))){
    ...
    dataSet$batch.cls[[label]] <- cls.nms;
    dataSet <<- dataSet;
...
ResetBatchData <- function(){
    dataSet$batch <- dataSet$batch.cls <- NULL;
    dataSet <<- dataSet;
}
...
PerformBatchCorrection <- function(imgName){
    ...
    for (i in 1:length(dataSet$batch)){
        ...
        cls <- as.character(dataSet$batch.cls[[i]]);
    ...
    batch.lbl <- factor(batch.lbls,levels=names(dataSet$batch), ordered=T);
    ...
    dataSet$batch.lbls <- batch.lbl;
    dataSet$cls.lbls <- cls.lbl;
    dataSet$adjusted.mat <- t(combat_edata);
    dataSet <<- dataSet;
...
PlotPCA.overview <- function(imgName, format="png", dpi=72, width=NA){
    ...
    pca <- prcomp(dataSet$adjusted.mat, center=T, scale=T);
    sum.pca<-summary(pca);
    var.pca<-sum.pca$importance[2,]; # variance explained by each PC
