...
get.csv <- function(patdb, 
 ...
 Chosen experimental conditon is '$exp.condition'.\n"))
  ...
  get.query = strwrap(fn$paste("select distinct i.filename, d.animal_internal_id, s.[$exp.condition] as label, d.sampling_date, i.mz, sum(i.intensity) as intensity
  ...
  cast.dt <- dcast.data.table(z.dt, animal_internal_id + sampling_date + label ~ mz, fun.aggregate = sum, value.var = "intensity") # what to do w/ duplicates? 
  print(cast.dt)
