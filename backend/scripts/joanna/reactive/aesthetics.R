...
observeEvent(input$color_ramp,{
  ...
  output$ramp_plot <- plotly::renderPlotly({
    ...
    global$functions$color.functions[[getOptions()$gspec]] <<- global$functions$color.functions[[input$color_ramp]]
    # change the current spectrum in user options file
    setOption(key="gspec", value=input$color_ramp)
    ...
    plotly::plot_ly(z = volcano,
                    colors = global$functions$color.functions[[getOptions()$gspec]](100),
                    type = "heatmap",
                    showscale=FALSE)  %>%
...
observeEvent(input$ggplot_theme,{
  ...
  setOption(key="gtheme", value=input$ggplot_theme)
  ...
  output$ggplot_theme_example <- renderPlot({
    p <- ggplot(mtcars) + geom_boxplot(aes(x = wt, y = mpg,
                                           colour = factor(gear)))
    p + global$functions$plot.themes[[getOptions()$gtheme]]()
  })
})
...
observeEvent(input$change_css, {
  ...
  setOption(key="col1", value=input$bar.col.1)
  setOption(key="col2", value=input$bar.col.2)
  setOption(key="col3", value=input$bar.col.3)
  setOption(key="col4", value=input$bar.col.4)

  # set default user font options
  setOption(key="font1", value=input$font.1)
  setOption(key="font2", value=input$font.2)
  setOption(key="font3", value=input$font.3)
  setOption(key="font4", value=input$font.4)

  # set default user font size options
  setOption(key="size1", value=input$size.1)
  setOption(key="size2", value=input$size.2)
  setOption(key="size3", value=input$size.3)
  setOption(key="size4", value=input$size.4)
